...
\author{GRI Project Contributors\thanks{Correspondence: \texttt{[correspondence email]}}}
...
\subsection{Metric Variants: SRI and VWRS}

\subsubsection{Strategic Representativeness Index (SRI)}
...
\subsubsection{Variance-Weighted Representativeness Score (VWRS)}

The VWRS adjusts for statistical reliability:
\begin{equation}\label{eq:vwrs}
    \text{VWRS} = 1 - \frac{\sum_{i=1}^K w_i \cdot |p_i - q_i|}{\sum_{i=1}^K w_i}
\end{equation}
where $w_i = q_i \cdot \text{SE}_i \cdot r_i$, with $\text{SE}_i = \sqrt{\hat{p}_i(1 - \hat{p}_i) / n_i}$ and reliability factor $r_i$ (default 1). The VWRS is a deliberate hybrid: unlike the pure representativeness metrics GRI and SRI, it incorporates stratum-level statistical reliability. It answers ``how consequential are the distributional gaps for inference?'' rather than ``how closely does the sample mirror the population?''
...
\subsection{Metric Variant Comparison}

Table~\ref{tab:variants} compares GRI, SRI, and VWRS for GD5. The gap between GRI (0.30) and VWRS (0.80) on Country $\times$ Gender $\times$ Age quantifies the difference between aspirational and achievable representativeness.

\begin{table}[h]
\centering
\caption{Comparison of GRI, SRI, and VWRS for GD5 ($N = 1{,}057$).}
\label{tab:variants}
\begin{tabular}{@{}lccc@{}}
\toprule
Dimension & GRI & SRI & VWRS \\
\midrule
Country $\times$ Gender $\times$ Age & 0.301 & 0.306 & 0.802 \\
Country $\times$ Religion & 0.484 & 0.424 & 0.969 \\
Country $\times$ Environment & 0.354 & 0.336 & 0.980 \\
...
\midrule
\textbf{Overall Average} & \textbf{0.619} & \textbf{0.602} & \textbf{0.922} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Benchmark uncertainty.} Population benchmarks are estimates with uncertainty. A fully Bayesian treatment propagating benchmark uncertainty into GRI confidence intervals remains future work.
...
The complementary SRI and VWRS span the survey lifecycle---design, execution, and evaluation. The framework is released as open-source software with authoritative population benchmarks. We invite the survey methodology and ML dataset quality communities to adopt, critique, and extend it.
